...
vgrdl <- vgrdl %>% 
  ...
  gather(c("DEW", "DEE"), key = "country_iso3c", value = "gdp") %>%
  mutate(gap = gdp/gdp_de) %>% # calculate East-West Gap
  select(country_iso3c, year, gap)

#_ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _ _
# Graph ----

# library(ggplot2)
# f <- ggplot(vgrdl, aes(x = year, y = gap, colour = country_iso3c))
# f + geom_line(aes(group = country_iso3c))
